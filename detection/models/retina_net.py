...
class RetinaNet(nn.Module):
    ...
    def forward(self, input):
        ...
        return struct(
            conf = torch.sigmoid(join_output(output.classification , self.num_classes)),
            locs = join_output(output.location, 4)
        )

    def parameter_groups(self, lr, fine_tuning=0.1):
        return self.pyramid.parameter_groups(lr, fine_tuning)
